function [t,cellh,controlAction,k] = Hepat_HR_control(feedBackInput,insultInput,startBal)
% Author: Daniel Cook
% Date: 01/01/2016
% Copyrighted under Creative Commons Share Alike
% To run, use: [t,cellh,controlAction,k] = Hepat_HR_control([1,1,1,0],1,[.05 .95]);
% k = parameter values, R1-R4 = Robustness per disturbance, OR = overall robustness

% This function runs the controlled model presented in "Cell network modeling to study
...
%     hillCoeff = 1;
%     feedBack3 = 2*k(9)^hillCoeff/(k(9)^hillCoeff+cellh(2)^hillCoeff); % Alternate feedback
end
if F(4) == 0
    feedBack4 = 0;
elseif F(4) == 1
...
if F(1) == 0
    feedBack1a = 1; feedBack1b = 1;
elseif F(1) == 1
    feedBack1a = (1-A2Plus/kCapPlus);
    feedBack1b = (1-A2Minus/kCapMinus);
end
if F(2) == 0
    feedBack2 = 1;
elseif F(2) == 1
    feedBack2 = kPenv*(1+x(3))/(A2Plus + kA*A2Minus);
end
if F(3) == 0
    feedBack3 = 1;
elseif F(3) == 1
    feedBack3 = kTenv*(1+x(4))/A2Minus;
%     hillCoeff = 1;
...
if F(4) == 0
    feedBack4 = 0;
elseif F(4) == 1
    feedBack4 = kRenew*(1/(1+exp(C1*A2Plus+C2)));
end
...
dxdt(3) = Kp1*(-dxdt(1)) + Ki1*(A2PlusIC-A2Plus); % Controller action (k_P_env)
dxdt(4) = Kp2*(-dxdt(2)) + Ki2*(A2MinusIC-A2Minus); % Controller action (k_T_env)
